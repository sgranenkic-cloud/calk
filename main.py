...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = (
        ...
        "Жмяк на кнопку в меню и поехали!"
    )
...
# ---------- 2.5) Круг 400 м из темпа ----------
async def lap400_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Введи темп на км в формате мм:сс, например 4:00:", reply_markup=kb_back())
    return 0  # single-step state

async def lap400_calc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.text == BACK_TEXT:
        return await go_back(update, context)
    try:
        pace_sec = parse_pace_to_sec_per_km(update.message.text)
        t400 = pace_sec * 0.4
        await update.message.reply_text(f"Круг 400 м при темпе {update.message.text}/км: {fmt_time(t400)}", reply_markup=kb_main())
        return ConversationHandler.END
    except Exception as e:
        await update.message.reply_text(f"Ошибка: {e}. Пример: 4:00", reply_markup=kb_back())
        return 0
...
async def calc_process(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    dist = get_val(text, "dist")
    pace = get_val(text, "pace")
    time = get_val(text, "time")
...
async def predict_calc(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    dist = get_val(text, "dist")
    time = get_val(text, "time")
    target = get_val(text, "target")

    if not (dist and time):
        return  # пропустим для других хэндлеров
...
def build_app():
    ...
    # Круг 400м из темпа
    conv_400 = ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^Круг 400 м из темпа$"), lap400_start),
                      CommandHandler("lap400", lap400_start)],
        states={
            0: [MessageHandler(filters.TEXT & ~filters.COMMAND, lap400_calc)]
        },
        fallbacks=[MessageHandler(filters.Regex(f"^{re.escape(BACK_TEXT)}$"), go_back)]
    )
    app.add_handler(conv_400)
